...
magicman::magicman(int lv_in, string name_in)
{
	...
	HPmax=140+7*(LV-1);		// HP increases 7 points per level
	HP=HPmax;
	MPmax=150+5*(LV-1);		// MP increases 5 points per level
	MP=MPmax;
	AP=40+8*(LV-1);			// AP increases 8 points per level
	DP=35+3*(LV-1);			// DP increases 3 points per level
	speed=25+1*(LV-1);		// speed increases 1 points per level
...
bool magicman::attack(player &p)
{
	...
	// Normal attack
	HPtemp=(int)((0.5*AP/p.DP)*AP*5/(rand()%4+10));
	cout<<name<<" uses wand, "<<p.name<<"'s HP decreases "<<HPtemp<<endl;
...
bool magicman::specialatt(player &p)
{
	if(MP<40)
	{
	...
	else
	{
		...
		//double hit=1;			
		//srand(time(NULL));		
		HPtemp=(int)(AP*0.8+20);		// not related to opponent's DP
		EXPtemp=(int)(HPtemp*1.5);		// special attack provides more experience
		cout<<name<<" uses final spark attack, "<<p.name<<"'s HP decreases "<<HPtemp<<endl;
		cout<<name<<" obtained "<<EXPtemp<<" experience."<<endl;
		p.HP=(int)(p.HP-HPtemp);
		EXP=(int)(EXP+EXPtemp);
		system("pause");
