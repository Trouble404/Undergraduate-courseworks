...
archer::archer(int lv_in, string name_in)
{
	...
	MP=MPmax;
	AP=40+5*(LV-1);			// AP increases 5 points per level
	DP=35+2*(LV-1);			// DP increases 2 points per level
	speed=30+3*(LV-1);		// speed increases 3 points per level
...
bool archer::attack(player &p)
{
	...
	{
		HPtemp=(int)((1.3*AP/p.DP)*AP*5/(rand()%4+10));		// opponent's HP decrement calculated based their AP/DP, and uncertain chance
	...
	// Normal attack
	HPtemp=(int)((1.5*AP/p.DP)*AP*5/(rand()%4+10));
	cout<<name<<" uses gun, "<<p.name<<"'s HP decreases "<<HPtemp<<endl;
...
bool archer::specialatt(player &p)
{
	...
	else
	{
		...
		//double hit=1;			
		//srand(time(NULL));		
		HPtemp=(int)(AP*0.2+20);		// not related to opponent's DP
		EXPtemp=(int)(HPtemp*1.5);		// special attack provides more experience
		cout<<name<<" uses block attack, "<<p.name<<"'s HP decreases "<<HPtemp<<endl;
		cout<<name<<" obtained "<<EXPtemp<<" experience."<<endl;
		p.HP=(int)(p.HP-HPtemp);
		EXP=(int)(EXP+EXPtemp);
		system("pause");
