...
{
	int i=1;
	book *p1,*p2,*p3;/*for set up linked list*/
	book *new,*last;
	FILE *fp;/*define a file poiner variable fp*/
	fp=fopen("librarybooks.txt","ab+");/*open a binary file namely librarybook*/ 
	...
	new=(book*)malloc(sizeof(book));/*distribute the dynamic address of p1,p2,p3 and new*/
	if(new==NULL)/*judge whether distribu the success or not*/
	{
		printf("Out of memory\n");
		exit(1);
	}
	printf("Please enter the name of the book:\n");
	fflush(stdin);/*clear buffer memory and accept carriage return*/  
	gets(new->book);/*read and store the book name use gets function*/
	...
	while(i==1)
	{
		...
		fflush(stdin);
		gets(new->ISBN);
		fp=fopen("librarybooks.txt","rb");/*read the binary file*/
		
		if(fp==NULL)/*judge the whether the file can open or not*/
		{
			printf("file is opened unseccessfully.\n");
			exit(1);
		}
		
		p2=head;/*let p2 pointer equal to head pointer*/
		while(!feof(fp))/*use while loop read the file all member*/
		{
			fread(p1,sizeof(book),1,fp);/*read the first character in the file and assign to p1*/
			p2->link=p1;/*read p1 value and store it to p2 next adress*/
			p2=p1;/*make p2 equal to p1*/
			p1=(book*)malloc(sizeof(book));/*distribute the dynamic address again*/ 
		}
		p2->link=NULL;/*judge whether finish distribute*/
		fclose(fp);
		p3=head;/*let p3 pointer equal to head pointer*/
	
		if(head==NULL||head->link==NULL)/*check if the adding book already exists in the library use if statement*/
		{
			printf("No books in the library!\n");
		}
		else
		{
			while(p3->link!=NULL)/*check all the message in the file use wile loop*/
			{
				p3=p3->link;/*read the message in linked list in order*//*read the message in linked list in order*/
				if(strcmp(p3->ISBN,new->ISBN)==0)/*judge the ISBN whether repeat or not*/
				{
	...
	printf("Please enter the subject of the book:\n");/*ask user input the subject of the book*/
	fflush(stdin);/*clear buffer memory and accept carriage return*/
	gets(new->subject);/*read and store the subject of the book use gets function*/
	...
	fflush(stdin);
	scanf("%d",&new->inventory);/*read and store the digit of the book use scanf function*/
	getchar();
	new->lend=0;/*define the lend book staring value equal to 0*/
	...
	fwrite(new,sizeof(book),1,fp);/*let new message write in file*/
	fclose(fp);
...
void delete_book(book *head)/*declare a function delete_book and use structure variable book define prode head*/
{
	char bookISBN[100];
	FILE *fp;
	book *p1,*p2,*p3,*p4,*p5;/*for set up linked list p1,p2,p3,p4 and p5*/
	int i=1;
	fp=fopen("librarybooks.txt","rb");/*read and open the binary file*/
    if(fp==NULL)/*judege this book whether exist or not*/
	{
		printf("There is no book in the system. Please add first!\n");
		exit(1);
	}
	p5=p4=p3=p1=p2=(book*)malloc(sizeof(book));/*distribute the dynamic address of p1,p2,p3,p4 and p5*/
	p1=head;
    while(!feof(fp))/*use while loop read the file all member*/
	{
		fread(p2,sizeof(book),1,fp);/*read the first character in the file and assign to p2*/
		p1->link=p2;
		p1=p2;
		p2=(book*)malloc(sizeof(book));/*distribute the dynamic address again*/
	}
	p1->link=NULL;
	fclose(fp);
    ...
    while(i==1)
	{
		...
		fflush(stdin);
		gets(bookISBN);
		p5=p4=head;/*let p4 pointer and p5 pointer equal to head pointer*/
		while(p4->link!=NULL)
		{
			p4=p4->link;
			if(strcmp(bookISBN,p4->ISBN)==0)/*judge the bookISBN whether is the user want to delete or not use strcmp function*/
			{
				i=0;
				p5->link=p4->link;/*connect p4 and p5 two linked list*/
				fp=fopen("librarybooks.txt","w");/*open and delete the information*/
				if(fp==NULL)
				{
					printf("Fail to open the file!\n");
					exit(1);
				...
				{
					p3=p3->link;
					fwrite(p3,sizeof(book),1,fp);/*let buffer information in p3 write in file*/
				}
				fclose(fp);
...
/*list all the books that have been borrow*/ 
